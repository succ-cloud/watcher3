...
/** Missing controller exports must not crash Express route registration on deploy. */
const productController = new Proxy(productControllerRaw, {
  get(target, prop) {
    ...
    console.warn(`itemsController.${name} is missing — route will return 500 until server is updated.`);
    return (req, res) => {
      res.status(500).json({
        success: false,
...
// Bulk create without images (photos added at warehouse when stock arrives)
router.post(
  '/bulk',
  verifyJWT,
  verifyRole(ROLES.ADMIN),
...
router.patch('/:id/arrive', verifyJWT, verifyRole(ROLES.ADMIN), productController.markProductArrived);

// ==================== PUBLIC ROUTES ====================

router.get('/search', productController.searchProductsByName);
router.post('/advanced-search', productController.advancedSearch);
router.get('/vendor-shop', verifyJWT, productController.getVendorShopProducts);
